...
import { SingleCountryApi } from "../../services/API";
...
const SingleCountryView = (props) => {
  ...
  const SingleCountryId = props.match.params.id;

  const singleCountryMethod = (id) => {
    //console.log(id);
    axios
      .get(SingleCountryApi + `${id}`)
      .then((response) => {
        if (response.data.status) {
          let countryData = response.data.countryName;
  ...
  useEffect(() => {
    singleCountryMethod(SingleCountryId);
  }, [SingleCountryId]);
      ...
      <div className="singleView">
        ...
        <div className="container-fluid">
          <div className="mt-3 forpadding1">
            <h3 className="ml-3">{countryName}</h3>
          </div>
          <div className="row">
            <div
